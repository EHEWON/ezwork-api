<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>You have received a new promotion proposal application</title>
  </head>
  <body>
    <p>You have received a new promotion plan application on the Cloud Partner Platform. Please log in to the system to view the details.</p>
    <p></p>
    <p>EHEWON, your global pan-industrial digital supply chain service platform.</p>
    <p>
      <a href="{{env('MALL_URL')}}" target="_blank">{{env('MALL_URL_NO_HTTP')}}</a>
    </p>
    <p>
      <a href="{{env('MALL_URL')}}" target="_blank">
        <img style="width: 128px;" src="{{env('FILE_HOST')}}/group1/M00/00/00/rBFIw2I65zKAHbkFAADxj2CkeEU808.png" />
      </a>
    </p>
  </body>
</html>